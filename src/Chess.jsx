...
function History({ moves }) {
  ...
  for (let index = 0; index < moves.length; index++) {
    ...
    result[moveCount - 1] +=
      `${move.type}${colMapping.get(move.target.col)}${rowMapping.get(move.target.row)} `;
  }
...
function Square({
  ...
  return (
    <div className={className} onClick={() => onClick?.(rowIdx, colIdx)}>
      {piece ? `${piece.type}` : ""}
    </div>
  );
}
...
function Game({ my_color }) {
    ...
    if (selectedSquare) {
      ...
      if (isValid) {
        ...
        if (
          ...
          Math.abs(target.col - selectedSquare.col) === 2
        ) {
          const isKingside = target.col === 6;
          const rookCol = isKingside ? 7 : 0;
          const newRookCol = isKingside ? 5 : 3;
          board[selectedSquare.row][newRookCol] =
            board[selectedSquare.row][rookCol];
        ...
        setMoveHistory([
          ...moveHistory,
          { type: sourcePiece.type, source: selectedSquare, target: target },
        ]);
        setBoard([...board]);
      ...
      const moves = validMoves(
        ...
        hypotheticalBoard[move.row][move.col] =
          hypotheticalBoard[newSelectedSquare.row][newSelectedSquare.col];
  ...
  const isSquareValid = (rowIdx, colIdx) => {
    return (
      validMoveList &&
      validMoveList.some((move) => move.row === rowIdx && move.col === colIdx)
    );
  };
    ...
    <div className="board-container">
      <div className="board-layout-container">
        <div className="rank-labels">
          {Array.from(rowMapping.values()).map((label) => (
            <div key={label}>{label}</div>
          ))}
        </div>
        <div className="file-labels">
          {Array.from(colMapping.values()).map((label) => (
            <div key={label}>{label}</div>
          ))}
        </div>
